...
export function buildBehavioralModel(studentId: string, sessions: any[]): BehavioralModel {
  // Phase 9: Closed-Loop Model Update includes verified graded exams
  const eligiblePersonalizationSessions = sessions
    .filter(s => (s.type === 'low_stakes' || s.reviewStatus === 'verified') && s.studentId === studentId && s.features)
  ...
  if (sessionCount >= 10) {
    ...
    // For prototype cold-start testing (<10), we'll do an in-sample or minimal split
    // e.g. 2 sessions -> 1 train, 1 calibrate just so the engine runs.
  ...
  const confidence = sessionCount === 0 ? 0 : Math.min(95, Math.round(100 - (100 / (sessionCount + 0.5))));
  const status: ModelStatus = sessionCount >= 10 ? 'active' : 'cold_start'; // As requested in Phase 5: 10 sessions for ACTIVE
  ...
  trainingSessions.forEach(session => {
    let fTime = 0, fPointer = 0, fScroll = 0, fRevision = 0, fPaste = 0;
