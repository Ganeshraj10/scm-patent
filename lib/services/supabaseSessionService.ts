...
export async function persistExamSessionToSupabase(
  ...
  try {
    ...
    // 1. Resolve student UUID from student identifier if needed
    let studentUuid = session.studentId;
    
    // Check if studentId is an institutional identifier (e.g. S001, STU-001) or UUID
...
// ─── 2. Fetch Student Sessions from Supabase ─────────────────────────────────
...
export async function fetchStudentSessionsFromSupabase(
  ...
  try {
    ...
    let studentUuid = studentIdentifier;
    let isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(studentIdentifier);
    ...
    if (!examErr && examData) {
      examData.forEach((es: any) => {
        ...
        sessions.push({
          ...
          hasBurstEvent: false,
          humanReviewLabel: es.review_status === 'review_required' ? 'flagged_mock' : 'clean_mock',
